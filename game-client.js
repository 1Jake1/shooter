...
const CONFIG = {
			...
			class SoundSystem {
				...
				playSmokeStart() {
					...
					for (let i = 0; i < len; i++)
						data[i] =
							(Math.random() * 2 - 1) * Math.pow(1 - i / len, 0.5) * 0.3;
				...
				_reverbTail(t, dur, vol) {
					...
					for (let i = 0; i < len; i++)
						data[i] =
							(Math.random() * 2 - 1) * Math.pow(1 - i / len, 1.5) * 0.3;
					const src = this.ctx.createBufferSource();
					src.buffer = buf;
					const g = this.ctx.createGain();
					g.gain.setValueAtTime(vol, t);
			...
			function spawnShellCasing() {
				...
				const rd = new THREE.Vector3(1, 0, 0).applyQuaternion(
					camera.quaternion,
				);
				const ud = new THREE.Vector3(0, 1, 0).applyQuaternion(
					camera.quaternion,
				);
				shell.position
					.copy(wp)
			...
			function spawnGrenade(type) {
				...
				const model =
					type === 'frag' ? createFragGrenade() : createSmokeGrenade();
				const wp = new THREE.Vector3();
				camera.getWorldPosition(wp);
				...
				const velocity = dir
					.clone()
					.multiplyScalar(cfg.throwForce * throwStrength);
			...
			function shoot() {
				...
				targets.forEach(t => {
					...
					t.group.traverse(child => {
						if (child.isMesh) targetMeshes.push({ mesh: child, target: t, type: 'bot' });
					});
				});
				...
				otherPlayers.forEach((player, playerId) => {
					player.group.traverse(child => {
						if (child.isMesh) targetMeshes.push({ mesh: child, player: player, playerId: playerId, type: 'player' });
					});
				});
				...
				if (
					...
					(!intersects.length ||
						wallIntersects[0].distance < intersects[0].distance)
				)
					createImpactSpark(
						wallIntersects[0].point,
						wallIntersects[0].face.normal,
					);
				createBulletTrail(
			...
			function reload() {
				...
				function updateReload() {
					...
					if (progress < 1) requestAnimationFrame(updateReload);
						...
						document
							.getElementById('reload-bar-container')
			...
			function checkCollision(newX, newZ) {
				...
				if (Math.abs(newX) > mapHalf - r || Math.abs(newZ) > mapHalf - r)
					return true;
				return false;
			}
			...
			function update() {
				...
				const scopeActive =
					currentWeapon === 'ssg' && weaponAnim.scopeProgress > 0.3;
				const bobX = scopeActive
					? 0
					: Math.sin(weaponAnim.bobPhase) * 0.006 * weaponAnim.bobAmount;
				const bobY = scopeActive
					? 0
					: Math.abs(Math.cos(weaponAnim.bobPhase)) *
						0.008 *
						weaponAnim.bobAmount;
				...
				if (weaponAnim.switchProgress >= 0) {
					...
					if (weaponAnim.switchProgress >= 1) weaponAnim.switchProgress = -1;
						...
						switchOffsetY =
							-0.3 * Math.sin(weaponAnim.switchProgress * Math.PI);
				}
				...
				if (currentWeapon === 'ssg') {
					...
					weaponAnim.scopeProgress +=
						(scopeT - weaponAnim.scopeProgress) * 0.15;
				...
				for (let i = explosions.length - 1; i >= 0; i--) {
					...
					if (e.isDebris) {
						...
						if (!e.dealt && t > 0.1) {
							...
							if (pdist < e.radius) {
								const dmg =
									CONFIG.grenades.frag.blastDamage *
									(1 - pdist / e.radius) *
									0.3;
...
// Создаем модель другого игрока
function createPlayerModel(playerId, team) {
	console.log('🎨 Creating player model for:', playerId, 'Team:', team);

	const g = new THREE.Group();
	const color = team === 'red' ? 0xff4444 : team === 'blue' ? 0x4488ff : 0x44ff44;
	...
	// Тело
	const body = new THREE.Mesh(new THREE.BoxGeometry(0.6, 1.5, 0.4), bodyMat);
	...
	// Руки
	const armL = new THREE.Mesh(new THREE.BoxGeometry(0.15, 1, 0.15), bodyMat);
	armL.position.set(-0.4, 0.7, 0);
	armL.castShadow = true;
	g.add(armL);

	const armR = new THREE.Mesh(new THREE.BoxGeometry(0.15, 1, 0.15), bodyMat);
	armR.position.set(0.4, 0.7, 0);
	armR.castShadow = true;
	g.add(armR);

	// Ноги
	const legL = new THREE.Mesh(new THREE.BoxGeometry(0.25, 0.8, 0.25), bodyMat);
	legL.position.set(-0.15, 0.1, 0);
	legL.castShadow = true;
	g.add(legL);

	const legR = new THREE.Mesh(new THREE.BoxGeometry(0.25, 0.8, 0.25), bodyMat);
	...
	console.log('✅ Player model created and added to scene. Children count:', g.children.length);
	console.log('   Position:', g.position.x, g.position.y, g.position.z);
...
function sendPosition() {
	...
	if (now - lastPositionSend > 1000) {
		console.log('Sending position:', posData.x.toFixed(2), posData.z.toFixed(2));
	}
}

// Получение позиций других игроков
socket.on('player_update', (data) => {
	const { playerId, x, y, z, yaw } = data;

	if (!otherPlayers.has(playerId)) {
		// Создаем нового игрока
		const playerModel = createPlayerModel(playerId, gameState.selectedTeam);
		otherPlayers.set(playerId, playerModel);
		console.log('✅ New player created:', playerId, 'Total players:', otherPlayers.size);
	}
	...
	if (player) {
		...
		// При первом обновлении устанавливаем позицию сразу
...
socket.on('player_damaged', (data) => {
	...
	if (targetId === socket.id) {
		...
		document.getElementById('health-value').textContent = Math.floor(playerState.health);
		...
		soundSystem.playHitSound();
		console.log(`💔 Received ${damage} damage from ${playerId}. Health: ${playerState.health}`);
...
socket.on('grenade_spawn', (data) => {
	...
	console.log(`💣 Grenade spawned by ${playerId}:`, type, 'at', position.x.toFixed(2), position.z.toFixed(2));
...
socket.on('grenade_explosion', (data) => {
	...
	console.log(`💥 Grenade exploded by ${playerId}:`, type, 'at', position.x.toFixed(2), position.z.toFixed(2));
